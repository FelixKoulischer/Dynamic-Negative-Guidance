...
def generate_batch(N_batch: int, diffusion: nn.Module, model: nn.Module, to_forget_model: nn.Module, guidance_type: str, device: str,
    ...
    Input:
            ...
            - guidance_type   : The guidance type, should be one of negative_prompting, safe_latent_diffusion, dynamic_negative_guidance
            ...
            - prior, Temp     : The additional hyperparameters for DNG (Required for DNG)
            - p_min, p_max    : Clamping of posterior (Optional for DNG)
            - offset          : Additional bias for DNG (Optional for DNG)
    Output:
            - A torch.Tensor of size (N_batch,C,H,W) containing the N_batch generated images
    '''
    set_seed(seed)
    assert guidance_type in ['safe_latent_diffusion', 'negative_prompting', 'dynamic_negative_guidance'], "Guidance type is not defined! Should be one of: safe_latent_diffusion, negative_prompting and dynamic_negative_guidance "
    ...
    if guidance_type == 'dynamic_negative_guidance':
        ...
        return gen_batch_DNG(N_batch, diffusion, model, to_forget_model, device, initial_noise, guidance_scale, prior, Temp, p_min, p_max,offset)
...
def gen_batch_SLD(N_batch: int, diffusion: nn.Module, model: nn.Module, to_forget_model: nn.Module, device: str, initial_noise: torch.Tensor,
    ...
    Output:
            - A torch.Tensor of size (N_batch,C,H,W) containing the N_batch generated images using Safe Latent Diffusion
    '''
    batch = initial_noise.to(device)
...
def gen_batch_DNG(N_batch: int, diffusion: nn.Module, model: nn.Module, to_forget_model: nn.Module, device: str, initial_noise: torch.Tensor,
                 guidance_scale: float, prior: float, Temp: float, p_min: float, p_max: float, offset: float) -> torch.Tensor:
    '''
    Function to generate a batch of images using Dynamic Negative Guidance (DNG) as guidance mechanism
    ...
    Output:
            - A torch.Tensor of size (N_batch,C,H,W) containing the N_batch generated images using Dynamic Negative Guidance
    '''
    batch = initial_noise.to(device)
    ...
    with torch.no_grad():
        ...
        for ti in reversed(range(0,diffusion.T)):
            ...
            p = compute_posterior_DNG(diffusion, ti, p, new_noised_x, noised_x, noise_pred, to_forget_noise_pred,
...
def compute_posterior_DNG(diffusion: nn.Module, t: int, p_prev: torch.Tensor, new_noised_x: torch.Tensor, noised_x: torch.Tensor,
    ...
    Input:
            ...
            - Temp                  : The temperature hyperparameter for DNG (Default = 1.)
    ...
    '''
    a_t, a_bar_t = diffusion.alphas[t], diffusion.alphas_cumprod[t]                      # Diffusion constants required for Dynamic Negative Guidance
